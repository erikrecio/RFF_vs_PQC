...
def main(weights_samples, bins_hist, circuit, dev):
    ...
    for i, nvec in enumerate(nvecs,0):
        _, _, _, f_inf, f_RKHS = fourier_coefficients_dD(qnode, nvec, circuit.dim_x)
        vec_f_inf.append(f_inf)
        vec_f_RKHS.append(f_RKHS)
        RKHS_over_inf.append(f_RKHS/f_inf)
        temp_nvecs.append(list(nvec))
        
        perc = (i+1)/(weights_samples**circuit.dim_w)*100
        et = time.time() - st
    ...
    file_name = f'{datetime.now().strftime("%d-%m-%Y %H-%M-%S")} - Norms_and_parameters of {circuit.name} - w={weights_samples}, n={circuit.n_qubits} , x={circuit.dim_x}'
    ...
    data = [vec_f_inf, vec_f_RKHS, RKHS_over_inf]
    ...
    for name, data in zip(names, data):
        ...
        plot_name = f'{name} of {circuit.name} - w={weights_samples}, n={circuit.n_qubits} , x={circuit.dim_x}'
        plt.title(plot_name)
        plt.xlabel(name)
        plt.xlim(left=0)
